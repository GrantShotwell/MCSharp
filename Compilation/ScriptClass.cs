                ...
                if(!char.IsWhiteSpace((char)current)) {
                    switch((char)current) {
                        ...
                        case '}':
                            ...
                            if(blocks.Count == 0) {
                                functions.Add(new ScriptMethod(root + "\\" + (string)alias, (string)type,
                        ...
                        case ')':
                                ...
                                for(int i = 0; i < wilds.Wilds.Count; i++) {
                                    if(wilds.Wilds[i].IsWilds) {
                                        ...
                                        if(Variable.Compilers.TryGetValue(ws[0], out var compiler)) {
                                            parameters[i] = compiler.Invoke(Access.Private, Usage.Default, ws[1], Compiler.CurrentScope, new ScriptWild[] { });
                            ...
                            break;
                        default:
                            //if(blocks.Count == 0) words[0] += current;
                            break;
                    }
                }
